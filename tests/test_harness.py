...
class OfflineTestHarness:
    ...
    async def run_scenario(self, scenario: TestScenario, claims: dict[str, Any]) -> TestResult:
        ...
        try:
            # Prepare context
            context = {'claims': claims, 'request_id': f'test-{scenario.name}-{int(time.time())}'}
            ...
            security_violations = await self._check_security_violations(
                scenario, response, tools_invoked, claims
            )

            # Check performance requirements
            performance_ok = self._check_performance_requirements(
                execution_time, response_quality, scenario
            )

            passed = (
                set(tools_invoked_names) == set(scenario.expected_tools)
                and response_quality
                >= scenario.performance_requirements.get('min_quality_score', 0.0)
    ...
    async def _check_security_violations(
        self, scenario: TestScenario, response: str, tools_invoked: list, claims: dict[str, Any]
    ) -> list[str]:
        ...
        for check in scenario.security_checks:
            if check == 'no_pii_leakage':
                ...
                if any(
                    re.search(pattern, scenario.input_query, re.IGNORECASE)
    ...
    def _check_performance_requirements(
        self, execution_time: float, quality_score: float, scenario: TestScenario
    ) -> bool:
    ...
    async def run_evaluation(
        self, test_users: list[dict[str, Any]], output_path: str | None = None
    ) -> EvaluationReport:
        ...
        report = EvaluationReport(
            ...
            performance_violations=len(
                [r for r in all_results if not r.metadata.get('performance_ok', True)]
            ),
...
async def run_quick_evaluation(
    agents: dict[str, BaseAgent], tool_registry: ToolRegistry, memory: Memory
) -> EvaluationReport:
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        print(
            'Test harness ready. Import and use run_quick_evaluation() or run_red_team_evaluation()'
        )
