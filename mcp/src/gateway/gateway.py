...
class MCPGateway:
    ...
    async def invoke_tool(
        ...
        try:
            ...
            await self.audit_logger.log_violation(
                tool_name=tool_name, args=args, claims=claims, violation=str(e)
            )
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f'Policy violation: {e!s}'
            )
        ...
        try:
            ...
            await self.audit_logger.log_rate_limit_exceeded(tool_name=tool_name, claims=claims)
            raise HTTPException(
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=f'Tool invocation failed: {e!s}',
            )

    async def _call_tool(
        self, tool_config: 'ToolConfig', args: dict[str, Any], claims: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        response = await self.http_client.post(
            f'{tool_config.mcp_endpoint}/execute', json=tool_args, timeout=30.0
        )
    ...
    async def _check_rate_limit(self, tool_name: str, claims: dict[str, Any]):
        ...
        rate_limit = self.config.rate_limits.per_tool.get(
            tool_name, self.config.rate_limits.default_rpm
        )
