...
* **Audyt narzędzi side-effect (ISSUE 019):** każda próba wywołania narzędzia `write`/`execute` przez `ToolRegistry.execute` — dozwolona, odrzucona przez RBAC lub zakończona błędem — jest trwale zapisywana przez skonfigurowany `AuditWriter` (`services/api-gateway/src/runtime/audit.py`), niezależnie od tego, czy krok pochodzi z planera LLM czy ścieżki fallback. Podgląd argumentów jest redagowany współdzielonym mechanizmem NEW-04. Sink wybiera `AUDIT_MODE`: domyślny `jsonl` włącza trwały zapis do pliku JSON-Lines (`AUDIT_LOG_PATH`). **Na wdrożonych warstwach** (`ENVIRONMENT` ∈ `production`/`prod`/`staging`/`stage`; `production` jest wartością domyślną, gdy `ENVIRONMENT` nie jest ustawione) brak `AUDIT_LOG_PATH` w tym trybie przerywa start serwisu (`AuditConfigError`) — audyt nie może po cichu spaść do trybu nietrwałego. Poza warstwami wdrożeniowymi (np. lokalny dev/test z `ENVIRONMENT=dev`) writer in-proces jest dozwolony, z ostrzeżeniem w logu.
* **Audyt na NATS JetStream (ISSUE 039):** jawnie włączany tryb `AUDIT_MODE=jetstream` zastępuje sink trwałym publikowaniem do NATS JetStream — wywołanie narzędzia z efektem ubocznym kończy się sukcesem dopiero po potwierdzeniu trwałego zapisu przez broker, a przy niedostępności/braku potwierdzenia (po ograniczonej liczbie prób i próbie zapisu do DLQ) kończy się odmową (fail-closed) na każdej warstwie. Usługa `services/auditor` konsumuje zdarzenia jako trwały konsument typu pull i potwierdza (ack) partię dopiero po trwałym zapisie do Elasticsearch i S3. Szczegóły: `audit/evidence/39_jetstream_durable_audit.md`.
