...
# Project: AstraDesk
# File: src/runtime/rag.py
# Website: https://www.astradesk.dev
# Repository: https://github.com/SSobol77/astradesk
#
# Description: Implements AstraDesk functionality for src/runtime/rag.py.
#
...
class RAG:
    ...
    def _prepare_chunks(self, chunks: Iterable[str | None]) -> list[str]:
        prepared: list[str] = []
        seen = set()
        for chunk in chunks:
            if chunk is None:
    ...
    async def retrieve_mmr(
        ...
        while remaining and len(selected) < k:
            ...
            for idx in remaining:
                ...
                if selected:
                    diversity = max(
                        _cosine_similarity(doc_vecs[idx], doc_vecs[j]) for j in selected
                    )
                else:
                    diversity = 0.0
        ...
        return [(candidates[i]['chunk'], score) for i, score in zip(selected, scores, strict=False)]
    ...
    async def upsert(self, source: str, chunks: Sequence[str]) -> int:
        ...
        inserted = 0
        try:
            for chunk, vector in zip(prepared, embeddings, strict=False):
