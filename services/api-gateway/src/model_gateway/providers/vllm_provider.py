...
class VLLMProvider(LLMProvider):
    ...
    def _handle_error(e: httpx.HTTPStatusError) -> ModelGatewayError:
        ...
        if 'error' in details and 'context_length' in details['error'].get('message', '').lower():
            return TokenLimitExceededError('Token limit exceeded', provider='vllm', details=details)
        ...
        try:
            ...
            if not content:
                raise ModelGatewayError(
                    'No content in vLLM response', provider='vllm', details=data
                )
            return content
            ...
            raise ModelGatewayError(f'Unexpected error with vLLM: {e!s}', provider='vllm') from e
    ...
    async def stream(
        ...
        try:
            ...
            raise ModelGatewayError(f'Streaming error with vLLM: {e!s}', provider='vllm') from e
