...
if not BEDROCK_MODEL_ID:
    logger.info(
        'BEDROCK_MODEL_ID env var not set; BedrockProvider imports, but chat()/stream() will raise until configured.'
    )
...
class BedrockProvider(LLMProvider):
    ...
    def _build_request_body(self, messages: Sequence[LLMMessage], params: dict[str, Any]) -> str:
        ...
        if self._model_id.startswith('anthropic.claude'):
            ...
            raise ModelGatewayError(
                f'Unsupported model family for {self._model_id}', provider='bedrock'
            )
    ...
    def _parse_response(self, response_body: bytes, family: str) -> str:
        ...
        try:
            ...
            raise ModelGatewayError(
                f'Invalid JSON in Bedrock response: {e}', provider='bedrock'
            ) from e
        ...
        if 'anthropic' in family:
            ...
            logger.warning(
                'Unknown model family for parsing', extra={'family': family, 'payload': payload}
            )
            return ''
        ...
        try:
            ...
            if error_code == 'ThrottlingException':
                raise ProviderOverloadedError(
                    error_msg, provider='bedrock', status_code=http_status
                ) from e
            ...
            raise ModelGatewayError(
                f'Unexpected error with Bedrock: {e!s}', provider='bedrock'
            ) from e
    ...
    async def stream(
        ...
        try:
            ...
            raise ModelGatewayError(
                f'Streaming error with Bedrock: {e!s}', provider='bedrock'
            ) from e
