...
class BillingAgent(BaseAgent):
    ...
    @staticmethod
    def _extract_invoice_id(query: str) -> str | None:
        patterns = [
    ...
    def _compose_response(
        ...
        for tool, result in zip(invoked_tools, tool_results, strict=False):
            if tool.name == 'get_invoice':
                ...
                lines.append(
                    f"• Wygenerowano raport użycia za okres **{tool.arguments.get('period')}**: {result}"
                )
    ...
    async def run(
        ...
        with self.tracer.start_as_current_span('billing.run') as span:
            ...
            final_response = self._compose_response(invoked_tools, tool_results, contextual_info)
