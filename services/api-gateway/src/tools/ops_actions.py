    ...
    *,
    ...
    with tracer.start_as_current_span('tool.ops.restart_service') as span:
        ...
        try:
            ...
            logger.warning(
                f"Access denied: missing role '{REQUIRED_ROLE_RESTART}' for service '{service}'"
            )
        ...
        try:
            ...
            if e.status == 404:
                logger.error(
                    f"Deployment '{service}' not found in namespace '{KUBERNETES_NAMESPACE}'"
                )
                return f"Error: Service '{service}' not found."
            elif e.status == 403:
                logger.critical(
                    f"Service Account lacks permissions to patch deployments in '{KUBERNETES_NAMESPACE}': {e.body}"
                )
