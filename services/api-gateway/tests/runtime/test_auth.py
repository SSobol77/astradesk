...
# Project: AstraDesk
# File: services/api-gateway/tests/runtime/test_auth.py
...
def _import_auth_with_env(monkeypatch) -> types.ModuleType:
    ...
    import core.src.astradesk_core.utils.auth as auth  # (import inside function)
...
async def test_verify_success_and_decode_args(monkeypatch):
    ...
    assert (
        isinstance(kwargs['options'], dict)
        and kwargs['options'].get('leeway') == auth.TIME_SKEW_LEEWAY
    )
...
async def test_key_rotation_retry_fetches_again_and_succeeds(monkeypatch):
    ...
    # jose mocks
    monkeypatch.setattr(
        auth.jwt, 'get_unverified_header', lambda t: {'kid': 'rotated', 'alg': 'RS256'}
    )
...
async def test_key_not_found_even_after_refresh_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        auth.jwt, 'get_unverified_header', lambda t: {'kid': 'missing', 'alg': 'RS256'}
    )
    monkeypatch.setattr(auth.jwk, 'construct', lambda key: {'constructed': key})
    # jwt.decode should never be called in this path
    monkeypatch.setattr(
        auth.jwt, 'decode', MagicMock(side_effect=AssertionError('should not decode'))
    )
