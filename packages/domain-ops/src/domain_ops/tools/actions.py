...
# Project: AstraDesk
# File: packages/domain-ops/src/domain_ops/tools/actions.py
# Website: https://www.astradesk.dev
# Repository: https://github.com/SSobol77/astradesk
#
# Description: Implements AstraDesk functionality for packages/domain-ops/src/domain_ops/tools/actions.py.
#
...
from astradesk_core.rbac import AuthorizationError, require_role
...
async def restart_service(service: str, *, claims: dict | None = None) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Odmowa dostępu dla restartu usługi '{service}': brak wymaganej roli '{REQUIRED_ROLE_RESTART}'."
        )
        raise
    ...
    try:
        ...
        logger.info(
            f"Pomyślnie zlecono restart wdrożenia '{service}' w przestrzeni nazw '{KUBERNETES_NAMESPACE}'."
        )
        ...
        if e.status == 404:
            logger.error(
                f"Nie udało się zrestartować usługi: wdrożenie '{service}' nie zostało znalezione w przestrzeni nazw '{KUBERNETES_NAMESPACE}'."
            )
            ...
            logger.error(
                f"Nieoczekiwany błąd API Kubernetes podczas restartu usługi '{service}': {e.body}",
                exc_info=True,
            )
            return f'Błąd: Wystąpił nieoczekiwany błąd API Kubernetes (status: {e.status}).'
    except Exception as e:
        logger.critical(
            f"Nieoczekiwany błąd podczas próby restartu usługi '{service}': {e}", exc_info=True
        )
