...
try:
    ...
    print(
        f'ImportError: {e}. Please install required libraries: pip install pypdf markdown-it-py beautifulsoup4'
    )
    sys.exit(1)
...
async def main() -> None:
    ...
    if not dataset_path.is_dir():
        logging.error(
            f"Error: Directory not found for agent '{args.agent_name}' at '{dataset_path}'"
        )
        return
    ...
    try:
        ...
        logging.error(
            f'An error occurred during RAG initialization or ingestion: {e}', exc_info=True
        )
        sys.exit(1)
